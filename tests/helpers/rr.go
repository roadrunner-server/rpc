...
// StartExpectNoListener boots the container and requires that nothing ends up
// listening on addr, which is how a disabled rpc section must behave.
func StartExpectNoListener(t *testing.T, cfgPath string, plugins []any, addr string, opts ...Option) {
	t.Helper()

	Start(t, cfgPath, plugins, opts...)

	d := net.Dialer{Timeout: probeDial}
	conn, err := d.DialContext(t.Context(), "tcp", addr)
	if err == nil {
		_ = conn.Close()
		t.Fatalf("something is listening on %s, the rpc plugin should be disabled", addr)
	}
}
